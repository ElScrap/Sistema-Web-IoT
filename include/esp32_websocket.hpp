...
void ProcessRequest(AsyncWebSocketClient *client, String request)
{
	...
	if(command == "restore"){
		...
		settingsResetRelays();
		if(configuracionSaveWiFi() && settingsSaveMQTT() && settingsSaveAdmin() && settingsSaveRelays()){
	...
	if (error)
	{
		Serial.println(F("Error al parsear el JSON RECIBIDO"));
		return;
	}
	const String emeil=jsonDoc["jemei"];
	const String modelo=jsonDoc["jmodelo"];
	const String mensaje=jsonDoc["jmensaje"];
	if (!emeil.isEmpty()&&!modelo.isEmpty()&&!mensaje.isEmpty()&&!mensaje.equals("null"))
	{
		topiccc=emeil+"/"+modelo+"/dispositivo/command";
		mensajemovil=mensaje;
	}
	const String mensajeGlobal1=jsonDoc["jmensajeGlobal"];//variable gobal
	if (!mensajeGlobal1.isEmpty()&&!mensajeGlobal1.equals("null"))
	{
		mensajeGlobal=mensajeGlobal1;
	}
}
...
String GetJson(){
	...
	jsonDoc["mqtt_server"] = mqttclient.connected() ? F(mqtt_server) : F("Servidor no conectado");
	...
	jsonDoc["modelomovil"] = modelomovil;
	jsonDoc["emeidispomovil"] = emeidispo;
	jsonDoc["mcoordenadas"] = mensajecoordenadas;
	jsonDoc["mestado"] = estadomovil;
